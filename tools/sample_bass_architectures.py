...
def build_eval_extra_command_template(
    *,
    set5_dir: str,
    set14_dir: str,
    bsd100_dir: str,
    ...
    return (
        "python tools/eval_extra_datasets.py "
        "--model-path {model_path} "
        "--sample-id {sample_id} "
        "--scale {scale} "
        f"--set5-dir {set5_dir} "
        f"--set14-dir {set14_dir} "
        f"--bsd100-dir {bsd100_dir} "
        f"--output-csv {output_dir}/{{sample_id}}_extra_datasets.csv"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--extra-set5-dir", default="/data/Set5")
    parser.add_argument("--extra-set14-dir", default="/data/Set14")
    parser.add_argument("--extra-bsd100-dir", default="/data/BSD100")
    parser.add_argument("--extra-eval-output-dir", default="results/zerocost_50_stratified_random/extra_datasets")
    parser.add_argument(
        "--max-epochs",
        type=int,
        default=1000,
...
def main() -> int:
    ...
    manifest_rows = build_manifest_rows(
        ...
        extra_eval_command_template=build_eval_extra_command_template(
            set5_dir=args.extra_set5_dir,
            set14_dir=args.extra_set14_dir,
            bsd100_dir=args.extra_bsd100_dir,
            output_dir=args.extra_eval_output_dir,
        ),
    )
    ...
    metadata = {
        ...
        "train_dir": args.train_dir,
        "val_dir": args.val_dir,
        "extra_set5_dir": args.extra_set5_dir,
        "extra_set14_dir": args.extra_set14_dir,
        "extra_bsd100_dir": args.extra_bsd100_dir,
        "extra_eval_output_dir": args.extra_eval_output_dir,
        "extra_eval_convention": (
            "RGB/full-image PSNR and SSIM with images normalized to [0, 1], "
            "HR cropped to dimensions divisible by scale, LR generated by TensorFlow "
            "resize using bicubic by default, and no border shaving. This matches the "
            "trainer metric convention more closely than Y-channel benchmark protocols."
        ),
        "max_epochs": max_epochs,
        "early_stopping_patience": args.early_stopping_patience,
        "reduce_lr_patience": args.reduce_lr_patience,
